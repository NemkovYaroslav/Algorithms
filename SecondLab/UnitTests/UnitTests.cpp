...
namespace UnitTests
{
	...
	{
	public:
		//ѕроверка конструтора по-умолчанию
		...
		//ѕроверка конструтора с параметрами
		...
		//ѕроверка копирующего конструтора
		...
		//ѕроверка копирующего оператора присваивани€
		...
		//ѕроверка перемещающего конструктора
		...
		//ѕроверка перемещающего оператора
		...
		//ѕроверка работы метода remove и класса итератора
		TEST_METHOD(RemoveAndIteratorTest)
		{
			...
			obj.remove(3);
			int i = 0;
			for (auto it = obj.iterator(); it.hasNext(); it.next()) {
